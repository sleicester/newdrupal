//90 - Enable disabling of certain event triggers for multiselect
// TODO 1 Wizard Widget Revisit + Plugin Version 2 Aria for certain components
...
//Fix Methods / Accessibility Methods, Helpers
(function ($, fix, aria, helpers, window) {
    fix.jQueryUIHelper = {
    ...
    fix.jQueryUIAutoComplete = function ($el, options) {
        var o = $.extend({
            windowResize: true,
            contentScroll: true,
    ...
    fix.jQueryUIComboBox = function ($el, options) {
        var o = $.extend({
            windowResize: true,
            contentScroll: true,
            contentAreaId: 'int-content',
    ...
    fix.jQueryUIDatePicker = function ($el, options) {
        var o = $.extend({
            windowResize: true,
            contentScroll: true,
            contentAreaId: 'int-content'
    ...
    helpers.setTristate = function ($master, noneChecked, allChecked) {
        if (noneChecked) {
            $master.prop({
                indeterminate: false,
                checked: false
    ...
    helpers.pluginfy = function (pluginName, namespace, constructor, duoArguments) {
        var instantiatedClass = "fn-" + pluginName;

        if (duoArguments) {
            $.fn[pluginName] = function ($element, options) {
                return this.each(function () {
                    if (!$(this).hasClass(instantiatedClass)) {
                        new namespace[constructor]($(this).addClass(instantiatedClass), $element, options);
                    }
                });
            };
        } else {
...
})(jQuery, OU.fix, OU.aria, OU.helpers, window);

//Widget Classes
(function ($, widgets, window) {
    widgets.Filter = function ($element, $items, options) {
    ...
    widgets.Filter.prototype = {
        ...
        filterKeyupHandler: function () {
            ...
            this.$element.keyup(function () {
                ...
                base
                    .filterSummary(filterValue, $relevantItems, $items)
                    .enableClearFilterButton(filterValue);
            });

            return this;
        },
        filterSummary: function (filterValue, $relevantItems, $items) {
            var o = this.options;

            if (o.summary) {
                o.$summary.html((filterValue != '') ? o.summaryHTML.replace('#relevant', $relevantItems.length.toString()).replace('#total', $items.length.toString()) : '');
            }

            return this;
        },
        clearFilterButton: function () {
            var o = this.options,
                $input = this.$element; //preserve this reference inside click handler by assigning to variable
        ...
        enableClearFilterButton: function (filterValue) {
            var o = this.options;

            if (o.clear && filterValue.length > 0 && o.$clearButton.prop('disabled')) {
                o.$clearButton.prop('disabled', false);
            }
            return this;
        },
        aria: function () {
            ...
            if (o.aria) {
                ...
                if (o.clear) {
                    o.$clearButton.attr('aria-controls', $input.attr('id'));
                }

                //Set aria on summary
                if (o.summary) {
                    o.$summary.attr('aria-live', 'polite');
                    $input.attr('aria-describedby', o.$summary.attr('id'));
                }
            }
        ...
        slaveChangeHandler: function () {
            ...
            this.$slaves.change(function () {
                if (o.tristate) {
                    ...
                    OU.helpers.setTristate($master, noneChecked, allChecked);
                }
                else {
    ...
    widgets.MultiSelect.prototype = {
        ...
        filter: function () {
            ...
            if (o.filter) {
                ...
                new widgets.Filter($filter, $items, { clear: true, $clearButton: $filterButton });  //Initialise filter object
    ...
    widgets.TreeToggle.prototype = {
        ...
        aria: function () {
            ...
            if (o.aria) {
                ...
                OU.aria.hidden($children);

                this.$container.on('click', o.triggerSelector, function () {
                    OU.aria.hidden($(this).next(o.childSelector));
                });
            }
    ...
    widgets.AccordionToggler.prototype = {
        ...
        init: function () {
            var base = this,
                ...
                $triggers = $accordion.children(base.options.triggerTag);

            $triggers.each(function () {
                var $trigger = $(this),
                    $panel = $trigger.next('div'),
                    iconExists = $trigger.children('i').length > 0,
                    panelVisible = $panel.hasClass(o.panelVisibleClass);

                if (panelVisible) {
                    $trigger.addClass(o.triggerActiveClass);
                }
                if (!iconExists) {
                    $trigger.prepend('<i class="' + o.iconClasses[(panelVisible) ? 0 : 1] + '"></i>');
                }
            });

            $accordion.on('click', o.triggerTag, function () {
                var $trigger = $(this),
                    $panel = $(this).next('div'),
                    $icon = $trigger.find('i');

                $panel.toggleClass(o.panelVisibleClass);

                $trigger.toggleClass(o.triggerActiveClass);

                if ($trigger.hasClass(o.triggerActiveClass)) {
                    $icon.removeClass(o.iconClasses[1]).addClass(o.iconClasses[0]);
                } else {
                    $icon.removeClass(o.iconClasses[0]).addClass(o.iconClasses[1]);
                }
            });
...
}(jQuery, OU.Widgets, window));

//Widget Plugins - Uses above classes to create plugins.
//Plugins protect against multiple instantiation and should be used by default
//For more control use the classes
(function ($, widgets) {
    OU.helpers.pluginfy('OUFilter', widgets, 'Filter', true);
    OU.helpers.pluginfy('OUButtonTextToggler', widgets, 'ButtonTextToggler');
    OU.helpers.pluginfy('OUCheckAll', widgets, 'CheckAll');
    OU.helpers.pluginfy('OUTreeCheckAll', widgets, 'TreeCheckAll');
    OU.helpers.pluginfy('OUStickyTableHeader', widgets, 'StickyTableHeader');
    OU.helpers.pluginfy('OUMultiSelect', widgets, 'MultiSelect');
    //TODO Wizard Plugin
    OU.helpers.pluginfy('OUSwitchButtons', widgets, 'SwitchButtons');
    OU.helpers.pluginfy('OUCheckboxButton', widgets, 'CheckboxButton');
    OU.helpers.pluginfy('OUListbox', widgets, 'Listbox');
    OU.helpers.pluginfy('OUTreeToggle', widgets, 'TreeToggle');
    OU.helpers.pluginfy('OUTabs', widgets, 'Tabs');

    //Core Components
    OU.helpers.pluginfy('OUDropDownMenu', widgets, 'DropDownMenu');
    OU.helpers.pluginfy('OUToggler', widgets, 'Toggler', true);
    OU.helpers.pluginfy('OUAccordionToggler', widgets, 'AccordionToggler');
    OU.helpers.pluginfy('OUThrobber', widgets, 'Throbber');
    OU.helpers.pluginfy('OUPlaceholder', widgets, 'Placeholder');
    //Global Handlers
    OU.helpers.pluginfy('OUAlerts', widgets, 'Alerts');
    OU.helpers.pluginfy('OUTipsyTooltip', widgets, 'TipsyTooltip');
    OU.helpers.pluginfy('OUFormHelp', widgets, 'FormHelp');
    OU.helpers.pluginfy('OUExpandableTables', widgets, 'ExpandableTables');
